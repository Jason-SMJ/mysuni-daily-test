...
**SMS 긴급 알림 (Fail 시)**
  - 최종 판정 결과에 비정상 항목 또는 시나리오 Fail이 존재하면 SMS API를 즉시 호출함
  - `SMS_ENABLED=true` 환경변수 설정만으로 활성화 가능함
  - Slack 알림 이후 순차 발송되어 이중 알림 체계 구성함
**유연한 실행 단위 제어**
...
# SMS 알림 (Fail 판정 시 발송)
SMS_ENABLED=true
# SMS_API_URL=https://mysuni.sk.com/api/notification/public/notifications?notificationId=CareerPlaywright-FAIL  # 기본값 사용 시 생략 가능
